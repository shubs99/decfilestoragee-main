...
# and applications. The evault network consists of two organizations with one
...
# Versions of fabric known not to work with the evault network
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $NONWORKING_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      fatalln "Local Fabric binary version of $LOCAL_VERSION does not match the versions supported by the evault network."
    fi

    infoln "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      fatalln "Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match the versions supported by the evault network."
    fi
  done
...
# certificates, and MSP folders that are needed to create the evault network in the
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Create evault 1 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-judge.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Create evault 2 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-lawyer.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/judge/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generate CCP files for evault 1 and evault 2"
...
# file for creating the network is "docker-compose-evault-net.yaml" in the ``docker``
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/judge/msp organizations/fabric-ca/judge/tls-cert.pem organizations/fabric-ca/judge/ca-cert.pem organizations/fabric-ca/judge/IssuerPublicKey organizations/fabric-ca/judge/IssuerRevocationPublicKey organizations/fabric-ca/judge/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/lawyer/msp organizations/fabric-ca/lawyer/tls-cert.pem organizations/fabric-ca/lawyer/ca-cert.pem organizations/fabric-ca/lawyer/IssuerPublicKey organizations/fabric-ca/lawyer/IssuerRevocationPublicKey organizations/fabric-ca/lawyer/fabric-ca-server.db'
...
COMPOSE_FILE_BASE=docker/docker-compose-evault-net.yaml
